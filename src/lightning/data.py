...
class MultiSceneDataModule(pl.LightningDataModule):
    ...
    def __init__(self, args, config):
        ...
        self.scan_img_resize = (config.EDM.TEST_RES_W,
                                config.EDM.TEST_RES_H)  # (640, 480)

        # MegaDepth options
        self.mgdpt_img_resize = config.EDM.TEST_RES_W  # 832
        self.mgdpt_img_pad = config.DATASET.MGDPT_IMG_PAD  # True
        self.mgdpt_depth_pad = config.DATASET.MGDPT_DEPTH_PAD  # True
        self.mgdpt_df = config.DATASET.MGDPT_DF  # 8
        self.coarse_scale = 1 / config.EDM.LOCAL_RESOLUTION  # 1/8
        ...
        # misc configurations
        self.parallel_load_data = getattr(
            args, "parallel_load_data", False)  # True for indoor
            ...
            if data_source == "ScanNet":
                datasets.append(
                    ScanNetDataset(
                        ...
                        pose_dir=pose_dir,
                        img_resize=self.scan_img_resize,
                        fp16=self.fp16,
                    )
                )
    ...
    def _build_concat_dataset_parallel(
        ...
        with tqdm_joblib(
            ...
            if data_source == "ScanNet":
                size = self.scan_img_resize
                fp = self.fp16
                datasets = Parallel(
                    n_jobs=math.floor(
                        len(os.sched_getaffinity(0)) *
                        0.9 / comm.get_local_size()
                    )
                )(
                    delayed(
